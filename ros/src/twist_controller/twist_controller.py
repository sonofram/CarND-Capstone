...
class Controller(object):
    ...
    def control(self, required_vel_linear,required_vel_angular,
        ...
        self.current_accel = self.sampling_rate * (self.past_vel_linear - current_vel_linear)
        # update

        self.past_vel_linear = current_vel_linear

        # use velocity controller compute desired accelaration
        desired_accel = self.sampling_rate * self.pid_vel_linear.step(velocity_error, self.delta_t)

        rospy.loginfo('TwistController.control: velocity error, current accel, desired accel: '+str(velocity_error)+ \
                      ' , '+str(self.current_accel)+' , '+str(desired_accel))

        # TODO think about emergency brake command
        if desired_accel > 0.0:
            if desired_accel < self.accel_limit:
                throttle = desired_accel - self.current_accel #self.accel_pid.step(desired_accel - self.current_accel, self.delta_t)
            else:
                throttle = self.accel_limit - self.current_accel #self.accel_pid.step(self.accel_limit - self.current_accel, self.delta_t)
            brake = 0.0
            self.low_pass_filter_accel.filt(throttle)
            throttle = self.low_pass_filter_accel.get()
        else:
            throttle = 0.0
